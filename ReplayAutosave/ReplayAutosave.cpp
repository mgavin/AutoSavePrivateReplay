#include "ReplayAutosave.h"
#include "pch.h"

BAKKESMOD_PLUGIN(
        ReplayAutosave,
        "Automatically saves replays of selected playlists",
        plugin_version,
        PLUGINTYPE_FREEPLAY)
...
void ReplayAutosave::onLoad() {
        auto enablePluginVar = cvarManager->registerCvar(
                "ranked_autosavereplay_playlists",
                "0",
                "save replays from specified playlists");
        enablePluginVar.addOnValueChanged([this](std::string, CVarWrapper cvar) { enable(cvar.getBoolValue()); });

        for (auto playlistPair : playlistStrings) {
                auto enableVar = cvarManager->registerCvar(
                        "ranked_autosavereplay_" + playlistPair.second,
                        "0",
                        "save replays from " + playlistPair.second);
...
void ReplayAutosave::enablePlaylist(int playlistID, bool enabled) {
...
void ReplayAutosave::enable(bool enabled) {
        if (enabled) {
                // hooks on a starting game
                gameWrapper->HookEventPost(
                        "Function GameEvent_Soccar_TA.Active.StartRound",
                        std::bind(&ReplayAutosave::startGame, this));
...
int ReplayAutosave::getPlaylistID() {
