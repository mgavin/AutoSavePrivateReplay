        ...
        for (const auto & plugin : *loaded_plugins) {
                if (plugin->_filename.compare("autosaveprivatereplay") == 0) {
                        ...
                        try {
                                ...
                                std::filesystem::remove(gameWrapper->GetBakkesModPath().append(
                                        "plugins/settings/AutoSavePrivateReplay.set"));
...
void AutosaveReplays::RenderSettings() {
        ...
        ImGui::NewLine();
        ImGui::PushStyleColor(ImGuiCol_Header, ImVec4(0.19f, 0.85f, 0.12f, 0.7f));
        ImGui::BeginColumns(
                "playlistselectables",
